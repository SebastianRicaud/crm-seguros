              ...
              <div className="space-y-3">
                {policies.map((p: any) => {
                  ...
                  return (
                    <div 
                      key={p.id} 
                      className="bg-white rounded-xl border-2 border-slate-200 hover:border-blue-300 transition-all p-4 cursor-pointer"
                    ...
                    >
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex flex-col gap-1 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                          ...
                          <button onClick={() => deletePolicy(p.id)} className="w-9 h-9 flex items-center justify-center rounded-lg bg-red-50 text-red-600 hover:bg-red-100 transition-colors text-sm" title="Eliminar póliza">🗑️</button>
                        </div>
                      </div>
                    </div>
                  );
...
function PolicyDetailView({ policy, client, onClose, onEdit, onRenew, onRefresh }: any) {
  const vigente = policy.policy_status === 'vigente' 
    ? true 
    : policy.policy_status === 'vencida' 
      ? false 
      ...
      <div className="space-y-4">
        ...
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-4">
          ...
          <div className="text-sm text-slate-700">
            ...
            {/* ✅ Muestra el NÚMERO REAL de la póliza anterior */}
            {policy.previous_policy_number && (
              <p className="text-xs text-blue-600 mt-2 bg-blue-50 px-3 py-1.5 rounded-lg inline-block border border-blue-200">
                🔄 Renovación de la póliza <strong>{policy.previous_policy_number}</strong>
              </p>
            )}
          </div>
        </div>
